...
async def attack_command(message):
    ...
    try:
        ...
        # Directly call the asynchronous function
        await process_attack_command(message)
    except Exception as e:
...
async def process_attack_command(message):
    try:
        ...
        if len(args) != 3:
            await bot.send_message(message.chat.id, "*Invalid command format. Please use: target_ip target_port duration*", parse_mode='Markdown')
            return
        ...
        attack_message = await bot.send_message(
            message.chat.id, 
        ...
        # Timer Countdown updated every second
        remaining_time = duration
        while remaining_time > 0:
            # Edit the original attack message to update the remaining time
            await bot.edit_message_text(
                f"*Attack started by {user_mention} 💥\n\nHost: {target_ip}\nPort: {target_port}\nTime: {remaining_time} seconds*",
                chat_id=message.chat.id,
